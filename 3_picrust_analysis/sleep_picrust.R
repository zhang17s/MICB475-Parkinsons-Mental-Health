...
abundance_daa_results_df <- pathway_daa(abundance = abundance_data_filtered %>% column_to_rownames("pathway"), metadata = Control_metadata, group = "Sleep_problems", daa_method = "DESeq2")
...
abundance_desc = abundance_desc[,-c(91:ncol(abundance_desc))]

# Generate pathway PCA plot
slp_control_pca <- pathway_pca(abundance = abundance_data_filtered %>% column_to_rownames("pathway"), metadata = Control_metadata, group = "Sleep_problems")
slp_control_pca

#Generate Log 2 Fold Change plot for yes vs. no samples
res =  DEseq2_function(abundance_data_filtered,Control_metadata,"Sleep_problems")
...
slp_control_log <- ggplot(data = sig_res, aes(y = reorder(description, sort(as.numeric(log2FoldChange))), x= log2FoldChange, fill = pvalue))+
...
ggsave(filename = "3_picrust_analysis/publication_picrust_figures/fig4_A_control_pca.png", slp_control_pca, 
       height = 6, width = 9)

ggsave(filename = "3_picrust_analysis/publication_picrust_figures/fig4_B_control_log.png", slp_control_log,
       height = 9, width = 11)
...
abundance_desc = inner_join(abundance,metacyc_daa_annotated_results_df, by = "feature")
abundance_desc$feature = abundance_desc$description
abundance_desc = abundance_desc[,-c(198:ncol(abundance_desc))]

# Generate pathway PCA plot
slp_PD_pca <- pathway_pca(abundance = abundance_data_filtered %>% column_to_rownames("pathway"), metadata = PD_metadata, group = "Sleep_problems")
slp_PD_pca
...
#Make log 2 fold change plot
sig_res <- sig_res[order(sig_res$log2FoldChange),]
slp_PD_log <- ggplot(data = sig_res, aes(y = reorder(description, sort(as.numeric(log2FoldChange))), x= log2FoldChange, fill = pvalue))+
  geom_bar(stat = "identity")+
  theme_bw()+
  labs(x = "Log2Fold Change", y="Metabolic Pathway", fill = "P Value") +
...
#saving log plot and pca figures
ggsave(filename = "3_picrust_analysis/publication_picrust_figures/fig4_C_control_pca.png", slp_PD_pca, 
       height = 6, width = 9)

ggsave(filename = "3_picrust_analysis/publication_picrust_figures/fig4_D_control_log.png", slp_PD_log,
       height = 6, width = 10)
